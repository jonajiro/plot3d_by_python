...
idestr = "0-3"
savepath = "train_data/"
savepath2 = "test_data/"+idestr+".bmp"
savepath3 = "output"+idestr+".bmp"
str_extension = ".bmp"
...
resolution_table = np.array(0.065)
...
img_target = cv2.imread(savepath2,1) # 画像読み込み
...
dism = np.zeros(len(text)) #非類似度
dism_ind = np.zeros([len(text),2],dtype=int) #非類似度
...
for z in range(len(text)):
    ...
    height = i.shape[0]
    width = i.shape[1]
    dism_tmp = np.zeros([len(resolution_table),len(filtcoef_table)])
        ...
        for k in range(len(filtcoef_table)):
            ...
            for y in range(height):
                for x in range(width):
                    dism_tmp[j,k] = dism_tmp[j,k] + (gray_target[x,y] - gray_base[x,y])*(gray_target[x,y] - gray_base[x,y])
            dism_tmp[j,k] = dism_tmp[j,k] / (width * height)
            print(dism_tmp[j,k])
    dism[z] = np.min(dism_tmp)
    idx = np.unravel_index(np.argmin(dism_tmp), dism_tmp.shape)
    dism_ind[z,0] = int(idx[0])
    dism_ind[z,1] = int(idx[1])
    print(dism_ind[z,0])
    print(dism_ind[z,1])
result_sort_ind = np.argsort(dism)
...
draw.font = ImageFont.truetype(font_path, font_size)  # font設定
draw.text((text_x, text_y), text[result_sort_ind[0]], text_color)  # pil_imageに直接書き込み
result_image = np.array(pil_image)  # OpenCV/numpy形式に変換
i = cv2.cvtColor(result_image, cv2.COLOR_RGB2GRAY)
height = i.shape[0]
width = i.shape[1]

img1 = cv2.resize(i , (int(width*resolution_table[dism_ind[result_sort_ind[0],0]]), int(height*resolution_table[dism_ind[result_sort_ind[0],0]])))
img2 = cv2.resize(img1 , (int(width), int(height)))
# img3 = cv2.cvtColor(img2, cv2.COLOR_RGB2GRAY)
gray_base = cv2.blur(img2,(filtcoef_table[dism_ind[result_sort_ind[0],1]],filtcoef_table[dism_ind[result_sort_ind[0],1]]))
# gray_base = img2
            # gray_base = cv2.equalizeHist(gray_base)
clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(3, 3))
gray_base = clahe.apply(gray_base)
...
np.savetxt('result'+idestr+'.csv', resultdata_sort_col_num, delimiter = ",", fmt = "%s")
